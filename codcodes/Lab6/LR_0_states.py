...
def get_data_LR_0_states(input_file) :
  global  graph, set_terminal, set_non_terminal, root, first_table, mini_item_set, follow_table, serial_productions, state_transitions, first_table

  data = read_file(input_file)
  #print data
  
  first_table, follow_table = follow.follow_and_l_recursion_removal(input_file) 
  #first and follow table obtained from follow method

  graph, root, set_terminal, set_non_terminal, serial_productions = elem_left_recursion.graph_root_terminal_non_terminal(input_file)
  #obtained the first and follow of the original of grammar from 
...
def generate_LR0_states_transition() :
  ...
  while not queue.empty() :
    ...
    state_transitions[count_of_states] = {}
    ...
    for prod_elem in mini_graph :
      ...
      print desired_elem
      print "into loop"
      print unique_next_elem[count_of_states]
...
def executer(input_file, output_file) :
  global graph, set_terminal, set_non_terminal, root, mini_item_set, follow_table, serial_productions, state_transitions, first_table
